...
contract CertificatesContract {
    ...
    event ZKProofVerified(uint256 certificateId, bytes32 proofHash, address verifier);
    ...
    modifier onlyValidCertificate(uint256 _certificateId) {
        require(_certificateId > 0 && _certificateId <= certificatesCounter, "ID de certificado invalido");
        require(certificates[_certificateId].isValid, "El certificado no es valido");
        _;
    }
    ...
    function _generateZKProofHash(bytes32 certificateHash) internal view returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                certificateHash,
    ...
    function revokeCertificate(uint256 _id) public onlyIssuerOrRecipient(_id) {
        require(_id > 0 && _id <= certificatesCounter, "ID de certificado invalido");
    ...
    function getCertificatesByIssuer(address _issuer) public view returns (uint256[] memory) {
        return issuerCertificates[_issuer];
    }

    // Generar datos para verificación externa (JSON/Link)
    function generateVerificationData(uint256 _id) public view returns (
        string memory verificationJSON
    ) {
        require(_id > 0 && _id <= certificatesCounter, "ID de certificado invalido");
        ...
        verificationJSON = string(abi.encodePacked(
            '{"id":', _uint2str(_id),
            ',"hash":"', _bytes32ToString(cert.certificateHash),
            '","easUID":"', _bytes32ToString(cert.easUID),
            '","recipient":"', _addressToString(cert.recipient),
            '","issuer":"', _addressToString(cert.issuer),
            '","valid":', cert.isValid ? "true" : "false",
            ',"timestamp":', _uint2str(cert.issuedAt), '}'
        ));
    }

    // Verificar certificado usando EAS directamente
    function verifyWithEAS(bytes32 _easUID) public view returns (bool isValid, bytes memory data) {
    ...
    function generateZKProof(uint256 _certificateId) public view returns (
        bytes32 proofHash,
        bytes32 commitment
    ) {
        require(_certificateId > 0 && _certificateId <= certificatesCounter, "ID invalido");
        require(certificates[_certificateId].recipient == msg.sender, "Solo el titular puede generar prueba ZK");
        
        proofHash = certificates[_certificateId].zkProofHash;
        commitment = keccak256(abi.encodePacked(proofHash, msg.sender, block.timestamp));
    }
}
